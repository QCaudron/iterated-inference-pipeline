...
void step_psr(struct s_X *p_X, double t, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc)
{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0;ac<N_AC;ac++) {
            ...
            _sf[0] = correct_rate(par[ORDER_d][routers[ORDER_d]->map[cac]],dt);
            _sf[1] = correct_rate(par[ORDER_k][routers[ORDER_k]->map[cac]],dt);

            
            _r[0] = _sf[0]+_sf[1];
            _r[1] = X[ORDER_I*N_CAC+cac]*par[ORDER_beta][routers[ORDER_beta]->map[cac]]*white_noise__0/covar[ORDER_N][nn][cac];
...

{
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][3]*X[ORDER_I*N_CAC+cac]))*dt + sqrt((_r[cac][3]*X[ORDER_I*N_CAC+cac]))*dem_sto__2[cac];
        }
...

{
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][3]*X[ORDER_I*N_CAC+cac]))*dt ;
        }
