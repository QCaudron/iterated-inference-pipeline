...
void eval_jac(gsl_matrix *Ft, const double *X, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc, struct s_group ***compo_groups_drift_par_proc, double t)
{
    ...
    for(cac=0; cac<N_CAC; cac++){
        ...
        _rj[cac][3] = X[ORDER_S*N_CAC+cac]*par[ORDER_r0][routers[ORDER_r0]->map[cac]]*par[ORDER_v][routers[ORDER_v]->map[cac]]/covar[ORDER_N][nn][cac];
        _rj[cac][4] = X[ORDER_I*N_CAC+cac]*par[ORDER_r0][routers[ORDER_r0]->map[cac]]*par[ORDER_v][routers[ORDER_v]->map[cac]]/covar[ORDER_N][nn][cac];
    }
