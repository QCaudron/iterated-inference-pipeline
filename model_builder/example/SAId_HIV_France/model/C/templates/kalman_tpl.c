...
void eval_jac(gsl_matrix *Ft, const double *X, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc, struct s_group ***compo_groups_drift_par_proc, double t)
{
    ...
    for(cac=0; cac<N_CAC; cac++){
        ...
        _rj[cac][0] = X[ORDER_S*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac];
        _rj[cac][1] = (par[ORDER_kA][routers[ORDER_kA]->map[cac]]);
        _rj[cac][2] = -X[ORDER_S*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac];
        _rj[cac][3] = -covar[ORDER_mu_d][nn][cac]-X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]-X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac];
        _rj[cac][4] = (par[ORDER_d][routers[ORDER_d]->map[cac]])+(par[ORDER_kI][routers[ORDER_kI]->map[cac]]);
        _rj[cac][5] = -covar[ORDER_mu_d][nn][cac]-(par[ORDER_d][routers[ORDER_d]->map[cac]])-(par[ORDER_kA][routers[ORDER_kA]->map[cac]])+X[ORDER_S*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac];
        _rj[cac][6] = (par[ORDER_d][routers[ORDER_d]->map[cac]]);
        _rj[cac][7] = 0;
        _rj[cac][8] = -X[ORDER_S*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac];
        _rj[cac][9] = -covar[ORDER_mu_d][nn][cac]-(par[ORDER_d][routers[ORDER_d]->map[cac]])-(par[ORDER_kI][routers[ORDER_kI]->map[cac]]);
        _rj[cac][10] = X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac];
    }
    ...
    for(c=0; c<N_C; c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            gsl_matrix_set(Ft, 0*N_CAC+cac, 0*N_CAC+cac, _rj[cac][3]);
            
            gsl_matrix_set(Ft, 0*N_CAC+cac, 1*N_CAC+cac, _rj[cac][8]);
            
            gsl_matrix_set(Ft, 0*N_CAC+cac, 2*N_CAC+cac, _rj[cac][2]);
            
            
            
            gsl_matrix_set(Ft, 1*N_CAC+cac, 0*N_CAC+cac, _rj[cac][10]);
            
            gsl_matrix_set(Ft, 1*N_CAC+cac, 1*N_CAC+cac, _rj[cac][5]);
            
            gsl_matrix_set(Ft, 1*N_CAC+cac, 2*N_CAC+cac, _rj[cac][0]);
            
            
            
            gsl_matrix_set(Ft, 2*N_CAC+cac, 0*N_CAC+cac, _rj[cac][7]);
            
            gsl_matrix_set(Ft, 2*N_CAC+cac, 1*N_CAC+cac, _rj[cac][1]);
            
            gsl_matrix_set(Ft, 2*N_CAC+cac, 2*N_CAC+cac, _rj[cac][9]);
    ...
    for(ts_unique=0; ts_unique < obs2ts[0]->n_ts_unique; ts_unique++) {
        for(stream=0; stream < obs2ts[0]->n_stream[ts_unique]; stream++) {
            ...
            for(n_cac=0; n_cac< obs2ts[0]->n_cac[ts_unique]; n_cac++) {
                ...
                gsl_matrix_set(Ft, N_PAR_SV*N_CAC+ts, 0*N_CAC+cac, _rj[cac][7]);
                
                gsl_matrix_set(Ft, N_PAR_SV*N_CAC+ts, 1*N_CAC+cac, _rj[cac][6]);
                
                gsl_matrix_set(Ft, N_PAR_SV*N_CAC+ts, 2*N_CAC+cac, _rj[cac][4]);
...
void eval_ht(struct s_kalman_update * p, double x, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc, int ts)
{
    ...
    gsl_vector_set(p->ht, N_PAR_SV*N_CAC +ts, par[ORDER_rep][routers[ORDER_rep]->map[ts]]*p_data->rep1[nn][ts]);
}
...
double var_f_x(double varx, double x, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc, int ts)
{
    ...
    double derf = par[ORDER_rep][routers[ORDER_rep]->map[ts]]*p_data->rep1[nn][ts];
    double der2f = 0;
...
void eval_Q_no_env_sto(gsl_matrix *Q, const double *X, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc, struct s_kalman_specific_data *p_kalman_specific_data, double t)
{
    ...
    for (cac=0; cac<N_CAC; cac++) {
        ...
        i = 0 * N_CAC + cac;
        j = 0 * N_CAC + cac;
        term = covar[ORDER_mu_b][nn][cac]*covar[ORDER_N][nn][cac]+covar[ORDER_mu_d][nn][cac]*X[ORDER_S*N_CAC+cac]+X[ORDER_S*N_CAC+cac]*(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac]);
        ...
        i = 1 * N_CAC + cac;
        j = 0 * N_CAC + cac;
        term = -X[ORDER_S*N_CAC+cac]*(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac]);
        ...
        i = 1 * N_CAC + cac;
        j = 1 * N_CAC + cac;
        term = covar[ORDER_mu_d][nn][cac]*X[ORDER_A*N_CAC+cac]+X[ORDER_A*N_CAC+cac]*(par[ORDER_d][routers[ORDER_d]->map[cac]])+X[ORDER_A*N_CAC+cac]*(par[ORDER_kA][routers[ORDER_kA]->map[cac]])+X[ORDER_S*N_CAC+cac]*(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac]);
        ...
        i = 2 * N_CAC + cac;
        j = 1 * N_CAC + cac;
        term = -X[ORDER_A*N_CAC+cac]*(par[ORDER_kA][routers[ORDER_kA]->map[cac]]);
        ...
        i = 2 * N_CAC + cac;
        j = 2 * N_CAC + cac;
        term = covar[ORDER_mu_d][nn][cac]*X[ORDER_I*N_CAC+cac]+X[ORDER_A*N_CAC+cac]*(par[ORDER_kA][routers[ORDER_kA]->map[cac]])+X[ORDER_I*N_CAC+cac]*((par[ORDER_d][routers[ORDER_d]->map[cac]])+(par[ORDER_kI][routers[ORDER_kI]->map[cac]]));
    ...
    */
    ...
    for(ts_unique=0; ts_unique < p_obs2ts->n_ts_unique; ts_unique++) {
		...
		term = -X[ORDER_A*N_CAC+cac]*(par[ORDER_d][routers[ORDER_d]->map[cac]]);
    ...
    */
    ...
    for(ts_unique=0; ts_unique < p_obs2ts->n_ts_unique; ts_unique++) {
		...
		term = -X[ORDER_I*N_CAC+cac]*((par[ORDER_d][routers[ORDER_d]->map[cac]])+(par[ORDER_kI][routers[ORDER_kI]->map[cac]]));
    ...
    */
    ...
    for(ts_unique=0; ts_unique < p_obs2ts->n_ts_unique; ts_unique++) {
        for(stream=0; stream < p_obs2ts->n_stream[ts_unique]; stream++) {
			...
			term += X[ORDER_A*N_CAC+cac]*(par[ORDER_d][routers[ORDER_d]->map[cac]])+X[ORDER_I*N_CAC+cac]*((par[ORDER_d][routers[ORDER_d]->map[cac]])+(par[ORDER_kI][routers[ORDER_kI]->map[cac]]));
...
void eval_Q_full(gsl_matrix *Q, const double *X, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc, struct s_kalman_specific_data *p_kalman_specific_data, double t)
{
    ...
    for (cac=0; cac<N_CAC; cac++) {
        ...
        i = 0 * N_CAC + cac;
        j = 0 * N_CAC + cac;
        term = covar[ORDER_mu_b][nn][cac]*covar[ORDER_N][nn][cac]+covar[ORDER_mu_d][nn][cac]*X[ORDER_S*N_CAC+cac]+pow(X[ORDER_S*N_CAC+cac],2)*pow(par[ORDER_sto][routers[ORDER_sto]->map[cac]],2)*pow(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac],2)+X[ORDER_S*N_CAC+cac]*(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac]);
        ...
        i = 1 * N_CAC + cac;
        j = 0 * N_CAC + cac;
        term = -pow(X[ORDER_S*N_CAC+cac],2)*pow(par[ORDER_sto][routers[ORDER_sto]->map[cac]],2)*pow(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac],2)-X[ORDER_S*N_CAC+cac]*(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac]);
        ...
        i = 1 * N_CAC + cac;
        j = 1 * N_CAC + cac;
        term = covar[ORDER_mu_d][nn][cac]*X[ORDER_A*N_CAC+cac]+X[ORDER_A*N_CAC+cac]*(par[ORDER_d][routers[ORDER_d]->map[cac]])+X[ORDER_A*N_CAC+cac]*(par[ORDER_kA][routers[ORDER_kA]->map[cac]])+pow(X[ORDER_S*N_CAC+cac],2)*pow(par[ORDER_sto][routers[ORDER_sto]->map[cac]],2)*pow(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac],2)+X[ORDER_S*N_CAC+cac]*(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac]);
...
void eval_Q_no_dem_sto(gsl_matrix *Q, const double *X, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc, struct s_kalman_specific_data *p_kalman_specific_data, double t)
{
    ...
    for (cac=0; cac<N_CAC; cac++) {
        ...
        i = 0 * N_CAC + cac;
        j = 0 * N_CAC + cac;
        term = pow(X[ORDER_S*N_CAC+cac],2)*pow(par[ORDER_sto][routers[ORDER_sto]->map[cac]],2)*pow(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac],2);
