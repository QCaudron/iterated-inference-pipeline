...
int step_lyap (double t, const double X[], double f[], void *params)
{
    ...
    */
    ...
    for(cac=0; cac<N_CAC; cac++){
        ...
        _rj[cac][3] = -covar[ORDER_mu_d][nn][cac]-X[ORDER_I*N_CAC+cac]*par[ORDER_r0][routers[ORDER_r0]->map[cac]]*par[ORDER_v][routers[ORDER_v]->map[cac]]*(par[ORDER_e][routers[ORDER_e]->map[cac]]*_sf[cac][0]+1)/covar[ORDER_N][nn][cac];
    }
    ...
    for(c=0; c<N_C; c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            for(i=0; i<(N_PAR_SV*N_CAC); i++) {
                ...
                f[N_PAR_SV*N_CAC+ (1*N_CAC+ cac)*N_PAR_SV*N_CAC +i] = +(_rj[cac][2])*X[N_PAR_SV*N_CAC+ (0*N_CAC+ cac)*N_PAR_SV*N_CAC +i]+(_rj[cac][0])*X[N_PAR_SV*N_CAC+ (1*N_CAC+ cac)*N_PAR_SV*N_CAC +i];
