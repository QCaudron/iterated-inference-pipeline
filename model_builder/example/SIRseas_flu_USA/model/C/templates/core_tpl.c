...
void step_psr(struct s_X *p_X, double t, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc)
{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0;ac<N_AC;ac++) {
            ...
            _sf[0] = sin(2.0*M_PI*(par[ORDER_d][routers[ORDER_d]->map[cac]]+t/ONE_YEAR_IN_DATA_UNIT));
            _sf[1] = correct_rate(par[ORDER_v][routers[ORDER_v]->map[cac]],dt);

            
            _r[0] = _sf[1];
            _r[1] = covar[ORDER_mu_d][nn][cac];
            _r[2] = X[ORDER_I*N_CAC+cac]*par[ORDER_r0][routers[ORDER_r0]->map[cac]]*par[ORDER_v][routers[ORDER_v]->map[cac]]*white_noise__0*(par[ORDER_e][routers[ORDER_e]->map[cac]]*_sf[0]+1)/covar[ORDER_N][nn][cac];

            sum = _r[2]*dt+_r[1]*dt;
if(sum>0.0){
one_minus_exp_sum = (1.0-exp(-sum));
p_calc->prob[ORDER_S][0] = one_minus_exp_sum*((_r[2]*dt)/sum);
...

{
    ...
    for(cac=0;cac<N_CAC;cac++){
        ...
        _r[cac][3] = X[ORDER_I*N_CAC+cac]*par[ORDER_r0][routers[ORDER_r0]->map[cac]]*par[ORDER_v][routers[ORDER_v]->map[cac]]*(par[ORDER_e][routers[ORDER_e]->map[cac]]*_sf[cac][0]+1)/covar[ORDER_N][nn][cac];
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc +=  + (_r[cac][3]*X[ORDER_S*N_CAC+cac]);
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][3]*X[ORDER_S*N_CAC+cac]) - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]))*dt + - sqrt((_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac]- sqrt((_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dem_sto__3[cac]+ sqrt((_r[cac][1]))*dem_sto__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][0]*X[ORDER_I*N_CAC+cac]))*dem_sto__2[cac]- sqrt((_r[cac][2]*X[ORDER_I*N_CAC+cac]))*dem_sto__4[cac]+ sqrt((_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dt + sqrt((_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac];
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][3]*X[ORDER_S*N_CAC+cac]) - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]))*dt + - sqrt((_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac]- sqrt((_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dem_sto__3[cac]+ sqrt((_r[cac][1]))*dem_sto__0[cac] + - (_r[cac][3]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][0]*X[ORDER_I*N_CAC+cac]))*dem_sto__2[cac]- sqrt((_r[cac][2]*X[ORDER_I*N_CAC+cac]))*dem_sto__4[cac]+ sqrt((_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac] + + (_r[cac][3]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dt + + sqrt((_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac]  + + (_r[cac][3]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][3]*X[ORDER_S*N_CAC+cac]) - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]))*dt + - (_r[cac][3]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dt + + (_r[cac][3]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dt  + + (_r[cac][3]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][3]*X[ORDER_S*N_CAC+cac]) - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]))*dt;
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dt;
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][3]*X[ORDER_S*N_CAC+cac]))*dt;
        }
