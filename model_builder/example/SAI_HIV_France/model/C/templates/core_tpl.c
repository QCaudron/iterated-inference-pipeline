...
void step_psr(struct s_X *p_X, double t, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc)
{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0;ac<N_AC;ac++) {
            ...
            _r[0] = _sf[0];
            _r[1] = covar[ORDER_mu_d][nn][cac];
            _r[2] = _sf[1];
            _r[3] = white_noise__0*(X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac]);

            sum = _r[3]*dt+_r[1]*dt;
if(sum>0.0){
one_minus_exp_sum = (1.0-exp(-sum));
p_calc->prob[ORDER_S][0] = one_minus_exp_sum*((_r[3]*dt)/sum);
p_calc->prob[ORDER_S][1] = one_minus_exp_sum*((_r[1]*dt)/sum);
...
p_calc->prob[ORDER_A][1] = one_minus_exp_sum*((_r[1]*dt)/sum);
p_calc->prob[ORDER_A][2] = 1.0 - p_calc->prob[ORDER_A][0] - p_calc->prob[ORDER_A][1];
}
else{
p_calc->prob[ORDER_A][0] = 0.0;
p_calc->prob[ORDER_A][1] = 0.0;
p_calc->prob[ORDER_A][2] = 1.0;
}

sum = _r[2]*dt+_r[1]*dt;
if(sum>0.0){
one_minus_exp_sum = (1.0-exp(-sum));
p_calc->prob[ORDER_I][0] = one_minus_exp_sum*((_r[2]*dt)/sum);
p_calc->prob[ORDER_I][1] = one_minus_exp_sum*((_r[1]*dt)/sum);
...
p_calc->prob[ORDER_I][2] = 1.0;
            ...
            plom_ran_multinomial(p_calc->randgsl, 3, (unsigned int) X[ORDER_A*N_CAC+cac], p_calc->prob[ORDER_A], p_calc->inc[ORDER_A][cac]);
...
X[ORDER_A*N_CAC+cac] = p_calc->inc[ORDER_A][cac][2] + p_calc->inc[ORDER_S][cac][0];
X[ORDER_I*N_CAC+cac] = p_calc->inc[ORDER_I][cac][2] + p_calc->inc[ORDER_A][cac][0];
    ...
    for(ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for(n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc +=  + p_calc->inc[ORDER_I][cac][0];
        }
        X[offset] += sum_inc;
        offset++;
    }
...

{
    ...
    for(cac=0;cac<N_CAC;cac++){
        ...
        _r[cac][0] = X[ORDER_A*N_CAC+cac]*par[ORDER_betaA][routers[ORDER_betaA]->map[cac]]/covar[ORDER_N][nn][cac]+X[ORDER_I*N_CAC+cac]*par[ORDER_betaI][routers[ORDER_betaI]->map[cac]]/covar[ORDER_N][nn][cac];
        _r[cac][1] = covar[ORDER_mu_b][nn][cac]*covar[ORDER_N][nn][cac];
        _r[cac][2] = (par[ORDER_kI][routers[ORDER_kI]->map[cac]]);
        _r[cac][3] = (par[ORDER_kA][routers[ORDER_kA]->map[cac]]);
        _r[cac][4] = covar[ORDER_mu_d][nn][cac];
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] =  - (_r[cac][0]*X[ORDER_S*N_CAC+cac]) - (_r[cac][4]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]);
            f[1*N_CAC+cac] =  - (_r[cac][3]*X[ORDER_A*N_CAC+cac]) - (_r[cac][4]*X[ORDER_A*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]);
            f[2*N_CAC+cac] =  - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) - (_r[cac][4]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_A*N_CAC+cac]);
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc +=  + (_r[cac][2]*X[ORDER_I*N_CAC+cac]);
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_S*N_CAC+cac]) - (_r[cac][4]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]))*dt + - sqrt((_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac]- sqrt((_r[cac][4]*X[ORDER_S*N_CAC+cac]))*dem_sto__4[cac]+ sqrt((_r[cac][1]))*dem_sto__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][3]*X[ORDER_A*N_CAC+cac]) - (_r[cac][4]*X[ORDER_A*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][3]*X[ORDER_A*N_CAC+cac]))*dem_sto__2[cac]- sqrt((_r[cac][4]*X[ORDER_A*N_CAC+cac]))*dem_sto__5[cac]+ sqrt((_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac];
            f[2*N_CAC+cac] = X[2*N_CAC+cac] +  ( - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) - (_r[cac][4]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_A*N_CAC+cac]))*dt + - sqrt((_r[cac][2]*X[ORDER_I*N_CAC+cac]))*dem_sto__3[cac]- sqrt((_r[cac][4]*X[ORDER_I*N_CAC+cac]))*dem_sto__6[cac]+ sqrt((_r[cac][3]*X[ORDER_A*N_CAC+cac]))*dem_sto__2[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][2]*X[ORDER_I*N_CAC+cac]))*dt + sqrt((_r[cac][2]*X[ORDER_I*N_CAC+cac]))*dem_sto__3[cac];
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_S*N_CAC+cac]) - (_r[cac][4]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]))*dt + - sqrt((_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac]- sqrt((_r[cac][4]*X[ORDER_S*N_CAC+cac]))*dem_sto__4[cac]+ sqrt((_r[cac][1]))*dem_sto__0[cac] + - (_r[cac][0]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][3]*X[ORDER_A*N_CAC+cac]) - (_r[cac][4]*X[ORDER_A*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][3]*X[ORDER_A*N_CAC+cac]))*dem_sto__2[cac]- sqrt((_r[cac][4]*X[ORDER_A*N_CAC+cac]))*dem_sto__5[cac]+ sqrt((_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac] + + (_r[cac][0]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[2*N_CAC+cac] = X[2*N_CAC+cac] +  ( - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) - (_r[cac][4]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_A*N_CAC+cac]))*dt + - sqrt((_r[cac][2]*X[ORDER_I*N_CAC+cac]))*dem_sto__3[cac]- sqrt((_r[cac][4]*X[ORDER_I*N_CAC+cac]))*dem_sto__6[cac]+ sqrt((_r[cac][3]*X[ORDER_A*N_CAC+cac]))*dem_sto__2[cac] ;
        }
    }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_S*N_CAC+cac]) - (_r[cac][4]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]))*dt + - (_r[cac][0]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][3]*X[ORDER_A*N_CAC+cac]) - (_r[cac][4]*X[ORDER_A*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt + + (_r[cac][0]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[2*N_CAC+cac] = X[2*N_CAC+cac] +  ( - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) - (_r[cac][4]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_A*N_CAC+cac]))*dt ;
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][2]*X[ORDER_I*N_CAC+cac]))*dt ;
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_S*N_CAC+cac]) - (_r[cac][4]*X[ORDER_S*N_CAC+cac]) + (_r[cac][1]))*dt;
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][3]*X[ORDER_A*N_CAC+cac]) - (_r[cac][4]*X[ORDER_A*N_CAC+cac]) + (_r[cac][0]*X[ORDER_S*N_CAC+cac]))*dt;
            f[2*N_CAC+cac] = X[2*N_CAC+cac] +  ( - (_r[cac][2]*X[ORDER_I*N_CAC+cac]) - (_r[cac][4]*X[ORDER_I*N_CAC+cac]) + (_r[cac][3]*X[ORDER_A*N_CAC+cac]))*dt;
        }
    }
