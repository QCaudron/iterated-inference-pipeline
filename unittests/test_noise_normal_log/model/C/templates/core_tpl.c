...
void proj2obs(struct s_X *p_X, struct s_data *p_data)
{
    ...
    for(n_ts_unique_o=0; n_ts_unique_o< (obs2ts[o])->n_ts_unique; n_ts_unique_o++) {
        ...
        for(n_cac_o_ts=0; n_cac_o_ts< (obs2ts[o])->n_cac[n_ts_unique_o]; n_cac_o_ts++) { //how many cities and age classes in this time serie
            ...
            sum_prev +=  + p_X->proj[ORDER_I*N_CAC+c*N_AC+ac];
        }
...
void step_psr(struct s_X *p_X, double t, struct s_par *p_par, struct s_data *p_data, struct s_calc *p_calc)
{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0;ac<N_AC;ac++) {
            ...
            _r[0] = X[ORDER_I*N_CAC+cac]*par[ORDER_r0][routers[ORDER_r0]->map[cac]]*par[ORDER_v][routers[ORDER_v]->map[cac]]*white_noise__0/covar[ORDER_N][nn][cac];
            _r[1] = _sf[0];
            _r[2] = covar[ORDER_mu_d][nn][cac];

            sum = _r[0]*dt+_r[2]*dt;
if(sum>0.0){
one_minus_exp_sum = (1.0-exp(-sum));
p_calc->prob[ORDER_S][0] = one_minus_exp_sum*((_r[0]*dt)/sum);
...
p_calc->prob[ORDER_I][2] = 1.0;
            ...
            p_calc->inc[ORDER_U][cac][0] = gsl_ran_poisson(p_calc->randgsl, (covar[ORDER_mu_b][nn][cac]*covar[ORDER_N][nn][cac]+1.0e-10)*dt);
...
X[ORDER_I*N_CAC+cac] = p_calc->inc[ORDER_I][cac][2] + p_calc->inc[ORDER_S][cac][0];
    ...
    for(ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for(n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc +=  + p_calc->inc[ORDER_I][cac][0] + p_calc->inc[ORDER_I][cac][1];
        }
        X[offset] += sum_inc;
        offset++;
    }
...

{
    ...
    for(cac=0;cac<N_CAC;cac++){
        ...
        _r[cac][2] = X[ORDER_I*N_CAC+cac]*par[ORDER_r0][routers[ORDER_r0]->map[cac]]*par[ORDER_v][routers[ORDER_v]->map[cac]]/covar[ORDER_N][nn][cac];
        _r[cac][3] = covar[ORDER_mu_b][nn][cac]*covar[ORDER_N][nn][cac]+1.0e-10;
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] =  - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) - (_r[cac][1]*X[ORDER_S*N_CAC+cac]) + (_r[cac][3]);
            f[1*N_CAC+cac] =  - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][2]*X[ORDER_S*N_CAC+cac]);
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc +=  + (_r[cac][0]*X[ORDER_I*N_CAC+cac]) + (_r[cac][1]*X[ORDER_I*N_CAC+cac]);
        }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc +=  + (_r[cac][2]*X[ORDER_S*N_CAC+cac]);
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) - (_r[cac][1]*X[ORDER_S*N_CAC+cac]) + (_r[cac][3]))*dt + - sqrt((_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac]- sqrt((_r[cac][1]*X[ORDER_S*N_CAC+cac]))*dem_sto__3[cac]+ sqrt((_r[cac][3]))*dem_sto__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][0]*X[ORDER_I*N_CAC+cac]))*dem_sto__2[cac]- sqrt((_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dem_sto__4[cac]+ sqrt((_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][0]*X[ORDER_I*N_CAC+cac]) + (_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dt + sqrt((_r[cac][0]*X[ORDER_I*N_CAC+cac]))*dem_sto__2[cac]+ sqrt((_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dem_sto__4[cac];
        }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dt + sqrt((_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac];
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) - (_r[cac][1]*X[ORDER_S*N_CAC+cac]) + (_r[cac][3]))*dt + - sqrt((_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac]- sqrt((_r[cac][1]*X[ORDER_S*N_CAC+cac]))*dem_sto__3[cac]+ sqrt((_r[cac][3]))*dem_sto__0[cac] + - (_r[cac][2]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dt + - sqrt((_r[cac][0]*X[ORDER_I*N_CAC+cac]))*dem_sto__2[cac]- sqrt((_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dem_sto__4[cac]+ sqrt((_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac] + + (_r[cac][2]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dt + + sqrt((_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dem_sto__1[cac]  + + (_r[cac][2]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) - (_r[cac][1]*X[ORDER_S*N_CAC+cac]) + (_r[cac][3]))*dt + - (_r[cac][2]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dt + + (_r[cac][2]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][0]*X[ORDER_I*N_CAC+cac]) + (_r[cac][1]*X[ORDER_I*N_CAC+cac]))*dt ;
        }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dt  + + (_r[cac][2]*X[ORDER_S*N_CAC+cac])*par[ORDER_sto][routers[ORDER_sto]->map[cac]]*white_noise__0[cac];
        }
...

{
    ...
    for(c=0;c<N_C;c++) {
        for(ac=0; ac<N_AC; ac++) {
            ...
            f[0*N_CAC+cac] = X[0*N_CAC+cac] +  ( - (_r[cac][2]*X[ORDER_S*N_CAC+cac]) - (_r[cac][1]*X[ORDER_S*N_CAC+cac]) + (_r[cac][3]))*dt;
            f[1*N_CAC+cac] = X[1*N_CAC+cac] +  ( - (_r[cac][0]*X[ORDER_I*N_CAC+cac]) - (_r[cac][1]*X[ORDER_I*N_CAC+cac]) + (_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dt;
        }
    }
    ...
    for (ts=0; ts<obs2ts[o]->n_ts_unique; ts++) {
        ...
        for (n_cac=0; n_cac<obs2ts[o]->n_cac[ts]; n_cac++) {
            ...
            sum_inc += ( + (_r[cac][2]*X[ORDER_S*N_CAC+cac]))*dt;
        }
